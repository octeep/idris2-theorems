...
public export
Uninhabited (Z = PS _) where
  uninhabited _ impossible

public export
Uninhabited (Z = NS _) where
  uninhabited _ impossible

public export
Uninhabited (PS _ = NS _) where
  uninhabited _ impossible

public export
Uninhabited (PS _ = Z) where
  uninhabited _ impossible

public export
Uninhabited (NS _ = Z) where
  uninhabited _ impossible

public export
Uninhabited (NS _ = PS _) where
  uninhabited _ impossible
...
DecEq ZZ where
  decEq Z Z = Yes Refl
  decEq (PS x) (PS y) = decEqCong $ decEq x y
  decEq (NS x) (NS y) = decEqCong $ decEq x y
  decEq Z (PS _) = No absurd
  decEq Z (NS _) = No absurd
  decEq (PS x) (NS y) = No absurd
  decEq (NS x) (PS y) = No absurd
  decEq (PS _) Z = No absurd
  decEq (NS _) Z = No absurd
...
public export
multOneRightNetural : (m : ZZ) -> m * 1 === m
multOneRightNetural Z = Refl
multOneRightNetural (PS k) =
  cong PS $ rewrite multZeroRightZero k in plusZeroRightNeutral k
multOneRightNetural (NS k) =
  cong NS $ rewrite multZeroRightZero k in plusZeroRightNeutral k

export
plusZeroRightNeutral : (m : ZZ) -> m + 0 === m
plusZeroRightNeutral Z = Refl
plusZeroRightNeutral (PS k) = Refl
plusZeroRightNeutral (NS k) = Refl

export
plusCommutative : (m, n : ZZ) -> m + n === n + m
...
export
castPlus : (m, n : Nat) -> the ZZ (cast (m + n)) = cast m + cast n
castPlus 0     n     = Refl
...
minusSuccSucc : (m, n : Nat) ->
  the ZZ (cast (S m) - cast (S n)) === cast m - cast n
...
export
plusAssociative : (m, n, p : ZZ) -> m + (n + p) === (m + n) + p
